...
![](https://gitee.com/alvin0216/cdn/raw/master/img/browser/macro/url-to-display.png)
...
3. **利用 IP 和服务器建立连接 TCP 连接**：
   <br />请求协议是 HTTP/1.1： 则经过 TCP 三次握手就可以建立连接了。默认端口 `80`
   <br />请求协议是 HTTPS：还需要经过 TSL 握手 建立安全连接。默认端口 `443`
